...
module Site =
    ...
    let URL_LANG (config: Config) lang =
        if config.MasterLanguage = lang then
            ""
        else
            lang
    ...
    let Page langopt (config: Config) (pageTitle: option<string>) hasBanner articles (body: Doc) =
        ...
        MainTemplate()
            .Language(
                if String.IsNullOrWhiteSpace langopt then
                    "en-GB"
                else if langopt = "en" then
                    "en-GB"
                else
                    "es-MX"
            )
...
#endif
            .NavbarOverlay(
                if hasBanner then "overlay-bar" else ""
            )
            .Head(head)
    ...
    let ArticlePage (config: Config) articles (article: Article) =
        ...
        MainTemplate
            ...
            // Sidebar
            .Sidebar(
                BlogSidebar config articles article
            )
            .Doc()
