...
module Helpers =
  ...
  let RSS_DATE(dt: DateTime) =
    dt.ToString("ddd, dd MMM yyyy HH:mm:ss UTC")
...
module Site =
  ...
  type MainTemplate =
    Templating.Template<
      "../Hosted/index.html",
      serverLoad=Templating.ServerLoad.WhenChanged
     >
  ...
  let ReadConfig() =
    ...
    if File.Exists config then
...

      {
        ...
        MasterUserDisplayName =
          Helpers.NULL_TO_EMPTY config.masterUserDisplayName
  ...
  let ReadArticles() : Articles =
    ...
    let ReadFolder user store =
      ...
      if Directory.Exists folder then
        ...
        |> List.fold
          (fun map (fullpath, fname, (year, month, day), slug, extension) ->
      ...
      .TopMenu(
        ...
        |> List.map (function
          | text, url, map when Map.isEmpty map ->
            MainTemplate.TopMenuItem().Text(text).Url(url).Doc()
          | text, _, children ->
            let items =
              children
              |> Map.toList
            ...
            .Categories(
              ...
              |> List.map(fun category ->
                MainTemplate
                  ...
                  .Name(category)
                  .Url(
                    Urls.CATEGORY category (URL_LANG config article.Language)
                  )
                  .Doc())
            )
            .Doc()
  ...
  let Main (config: Config ref) (articles: Articles ref) =
                ...
                |> List.map(fun category ->
                  MainTemplate
                    ...
                    .Url(
                      Urls.CATEGORY
                        category
                        (URL_LANG config.Value article.Language)
                    )
                    .Doc())
                |> Doc.Concat
            )
    ...
    let ARTICLE(user, p: string) =
      ...
      if articles.Value.ContainsKey key then
        ...
        |> List.map fst
        |> sprintf
          "Trying to find page \"%s\" (with key=\"%s\"), but it's not in %A"
          p
          page
        |> Content.Text
          ...
          WriteBody =
            fun stream ->
              ...
              let articles =
                articles.Value
                |> Map.toList
                |> List.sortByDescending(fun (_, article: Article) ->
                  article.Date.Ticks)
                ...
                X (ns + "feed") [] [
                  ...
                  for ((user, slug), article) in articles do
                    X (ns + "entry") [] [
                      ...
                      X (ns + "updated") [] [
                        TEXT <| Helpers.ATOM_DATE article.Date
                      ]
                      X (ns + "content") [
              ...
              let doc =
                X (N "rss") [ "version" => "2.0" ] [
                  X (N "channel") [] [
                    ...
                    X (N "lastBuildDate") [] [
                      Helpers.RSS_DATE DateTime.UtcNow
                    ]
                    for ((user, slug), article) in articles do
                      X (N "item") [] [
...
type Website() =
  ...
  interface IWebsite<EndPoint> with
    ...
    member this.Actions =
      ...
      let users = articles |> List.map(fst >> fst) |> Set.ofList |> Set.toList
